...
def HLS_CMR_STAC(hls_data, lim=100, aws=False):
...
def build_xr(stac_dict, lut=lut):
    try:
...
def get_hls(hls_data={}, lut=lut, lim=100, aws=False):   
    # run functions
    catalog = HLS_CMR_STAC(hls_data, lim, aws)
    da  = build_xr(catalog, lut)
    return da
